...
public abstract class AGateway {
     ...
     *
     * @param value True to enable the gateway for inbound messaging.
     */
     ...
     *
     * @param value True to enable the gateway for outbound messaging.
     */
     ...
     *
     * @param myProtocoll The protocol to be used.
     * @see Protocols
     * @see #getProtocol
     */
     ...
     *
     * @param myStatus The new gateway status.
     * @see GatewayStatuses
     */
     ...
     *
     * @param myFrom The originator string.
     * @see #getFrom()
     */
     ...
     *
     * @param msg The OutboundMessage object to be checked.
     ...
     * Query the gateway for message delivery status.
     *
     * @param refNo The reference number of a previously sent message to be
     *              checked.
     * @return The delivery status. This is interpreted and mapped to the
     * standard SMSLib status codes. For detailed information, check
     * method getDeliveryErrorCode().
    ...
    private class QueueManager extends AServiceThread {
        ...
        public void process() throws Exception {
            ...
            try {
                if (getStatus() == GatewayStatuses.STARTED) {
                    ...
                    if (msg != null) {
                        if (Service.getInstance().getQueueSendingNotification() != null)
                            Service.getInstance().getQueueSendingNotification().process(getMyself(), msg);
                        try {
                            if (!sendMessage(msg)) {
                ...
                if ((msg != null) && (msg.getMessageStatus() != MessageStatuses.SENT))
                    Service.getInstance().getQueueManager().queueMessage(msg);

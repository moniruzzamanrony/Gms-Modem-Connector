...
public class ClickatellHTTPGateway extends HTTPGateway {
     ...
     *
     * @param mySecure True for HTTPS, false for plain HTTP.
     */
    ...
    public float queryBalance() throws TimeoutException, GatewayException, IOException, InterruptedException {
        ...
        if (response.get(0).indexOf("Credit:") == 0)
            return Float.parseFloat(response.get(0).substring(response.get(0).indexOf(':') + 1));
        return -1;
    }
        ...
        try {
            ...
            if (msg.getType() == MessageTypes.OUTBOUND) {
                if ((msg.getSrcPort() != -1) || (msg.getDstPort() != -1))
                    request.add(new HttpHeader("udh", msg.getPduUserDataHeader(), false));
            ...
            if ((getFrom() != null && getFrom().length() != 0) || (msg.getFrom() != null && msg.getFrom().length() != 0))

...
public abstract class AModemDriver {
        ...
        synchronized (getSYNCCommander()) {
            try {
                ...
                if (!getGateway().getATHandler().setIndications()) {
                    ...
                    if (getGateway().getATHandler().getIndications().getMode().equals("0"))
                        getCnmiEmulationProcessor().enable();
                }
    ...
    public String getResponse(AsyncEvents eventResponse) throws GatewayException, TimeoutException, IOException, InterruptedException {
        ...
        try {
            while (true) {
                ...
                while (true) {
                    ...
                    if (System.getProperty("smslib.dumpqueues") != null)
                        Logger.getInstance().logDebug("OUT READER QUEUE : " + (int) c + " / " + (char) c, null, getGateway().getGatewayId());
            ...
            if (getGateway().getATHandler().isUnsolicitedResponse(buffer.toString())) {
                ...
                if (event == eventResponse && eventResponse != AsyncEvents.NOTHING) {
                    return buffer.toString();
                }
                if ((event == AsyncEvents.INBOUNDMESSAGE) || (event == AsyncEvents.INBOUNDSTATUSREPORTMESSAGE) || (event == AsyncEvents.INBOUNDCALL) || (event == AsyncEvents.USSDRESPONSE))
    ...
    private class CharQueue {
        ...
        public synchronized void put(byte c) {
            ...
            if (System.getProperty("smslib.dumpqueues") != null)
                Logger.getInstance().logDebug("IN READER QUEUE : " + (int) c + " / " + (char) c, null, getGateway().getGatewayId());
            notifyAll();
        }
            ...
            while (true) {
                try {
                    ...
                    if (getGateway().getStatus() == GatewayStatuses.STARTED)
                        Logger.getInstance().logWarn("Ignoring InterruptedException in Queue.get().", null, getGateway().getGatewayId());
                    else {

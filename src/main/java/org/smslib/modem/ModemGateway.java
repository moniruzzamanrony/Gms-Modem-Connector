...
public class ModemGateway extends AGateway {
    ...
    private void checkMpMsgList(Collection<InboundMessage> msgList) throws TimeoutException, GatewayException, IOException, InterruptedException {
        ...
        for (List<InboundMessage> remainingList : this.mpMsgList) {
            for (InboundMessage msg : remainingList) {
                if (getAgeInHours(msg.getDate()) > Service.getInstance().getSettings().HOURS_TO_ORPHAN)
                    if (Service.getInstance().getOrphanedMessageNotification() != null)
                        if (Service.getInstance().getOrphanedMessageNotification().process(Service.getInstance().getGateway(msg.getGatewayId()), msg) == true)
                            deleteMessage(msg);
            }
        }
    }
     ...
     *
     * @param ussdCommand the USSD command to send
     * @param interactive whether the USSD session should be left open
     * @return the network's response to the command
     * @throws GatewayException
     * @throws TimeoutException
     * @throws IOException

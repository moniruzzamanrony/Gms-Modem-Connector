...
public class IPModem extends AGateway {
    ...
    public void create() throws Exception {
        ...
        if (getProperties().getProperty(propName + "protocol").equalsIgnoreCase("pdu"))
            getGateway().setProtocol(Protocols.PDU);
        else if (getProperties().getProperty(propName + "protocol").equalsIgnoreCase("text"))
            getGateway().setProtocol(Protocols.TEXT);
        else throw new Exception("Incorrect parameter: " + propName + "protocol");
        if (getProperties().getProperty(propName + "ipprotocol").equalsIgnoreCase("text"))
            ((ModemGateway) getGateway()).setIpProtocol(IPProtocols.TEXT);
        else if (getProperties().getProperty(propName + "ipprotocol").equalsIgnoreCase("binary"))
        ...
        if (getProperties().getProperty(propName + "smsc_number", "").length() > 0)
            ((ModemGateway) getGateway()).setSmscNumber(getProperties().getProperty(propName + "smsc_number"));
    }
}
